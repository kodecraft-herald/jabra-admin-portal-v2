...
use crate::components::{common::{components::modals::{ErrorModal, SuccessModalRefetch}, functions::{helpers::{calculate_time_difference, change_day, extract_date_as_string, get_expiry}, precision::{format_with_specs, RoundType}, utils::fetch_unified_configuration}, models::{
    ...
    }}, quote_builder::future::model::{sort, FuturesQuote, QuoteSort}};

use super::model::{ExtractedFuturesQuote, FuturesQuoteHistory};

#[allow(non_snake_case)]
#[component]
pub fn FuturesGeneratorActivity() -> impl IntoView {
        ...
        >
            {
                config_resource.and_then(|conf| {
                    ...
                    view!{
                        <Futures config = configuration currency_pair = currency_pair/>
                    }.into_view()
                })
            }
        </Suspense>
...
pub fn Futures(
    ...
    let rfq_type = create_rw_signal("".to_string());
    let expiry_date = create_rw_signal(Utc::now() + Duration::days(6));
    let current_date = create_rw_signal(Utc::now());
    let expiry_date_string = create_rw_signal("".to_string());

    let expiry_date_str_value = move || extract_date_as_string(expiry_date.get());
    let current_date_str_value = move || extract_date_as_string(current_date.get());

    let notional_amount = create_rw_signal(0.0);
    let price = create_rw_signal(1.0);
    let quote_expiry = create_rw_signal("".to_string());
    ...
    let trade_quotes = RwSignal::new(Vec::<FuturesQuote>::default());

    let on_add_quote = move || {
        let expiry_date_time = get_expiry(expiry_in_min.get());
        if rfq_type.get() == "Perpetual".to_string() {
            let group_id = uuid::Uuid::new_v4().to_string();
            let jabra_quote = FuturesQuote {
                group_id: group_id.clone(),
                rfq_type: rfq_type.get(),
    ...
    create_effect(move |_| {
        ...
        log::info!("Rfq Type: {}", rfq_type.get());
        ...
        <div class = "flex flex-row gap-2 mt-2 sm:flex:wrap px924:flex-nowrap">
            <div class = "flex-1 sm:basis-full lg:basis-1/5 lg:grow-0 rounded-md px-4 py-1 bg-base-300">
                <div class = "flex flex-col flex-initial">
                    <label class = "font-light text-sm">Type</label>
                    <select class = "select-sm text-xs text-success block w-full rounded hover:shadow-sm hover:shadow-success bg-base-100 shadow-md" name="rfq_type"
                        on:change = move |e| {
                            let val: String = event_target_value(&e);
                            if has_filled.get() == false {
                                if val.clone() == "Future".to_string() {
                                    next_step.set(2)
                                } else {
                                    next_step.set(3)
                                }
                    ...
                    >
                        <option prop:selected = move || rfq_type.get() == "".to_string() disabled>Select Type</option>
                        <option prop:selected = move || rfq_type.get() == "Future".to_string() value="Future".to_string()>Future</option>
                        <option prop:selected = move || rfq_type.get() == "Perpetual".to_string() value="Perpetual".to_string()>Perpetual</option>
                    </select>

                    {
                        move || {
                            if next_step.get() < 2 {
                                view! {
                                    <div class = ("hidden", move || rfq_type.get() == "Perpetual".to_string()) class = "grid gap-1 mt-1">
                                ...
                                view! {
                                    <label class = ("hidden", move || rfq_type.get() == "Perpetual".to_string()) class = "font-light text-sm">Expiry</label>
                                    <input class = ("hidden", move || rfq_type.get() == "Perpetual".to_string()) class = "input-sm text-xs text-success block w-full rounded hover:shadow-sm hover:shadow-success bg-base-100 shadow-md" type="date" id="expiry" name="expiry" prop:value = expiry_date_string min = current_date_str_value()
                                        on:change = move |event| {
                                            let val: String = event_target_value(&event);
                                            let exp_date_time = change_day(expiry_date.get(), val).unwrap_or(expiry_date.get());
                                            expiry_date.set(exp_date_time);
                                            expiry_date_string.set(expiry_date_str_value());
                                ...
                                view! {
                                    ...
                                    <input class = "input-sm text-xs text-success block w-full rounded hover:shadow-sm hover:shadow-success bg-base-100 shadow-md" type="number" name="deposit_amount" prop:value = notional_amount prop:min = move || currency.get().order_size() prop:step = move || currency.get().tick_size()
                                        on:change = move |e| {
                                            ...
                                            if has_filled.get() == false && next_step.get() <= 5 {
                                                next_step.set(5)
                                            } else {
                                                next_step.update(|value| *value += 0)
                                            }
                                ...
                                view! {
                                    ...
                                    <input class = "input-sm text-xs text-success block w-full rounded hover:shadow-sm hover:shadow-success bg-base-100 shadow-md" type="number" name="price" prop:value = price prop:min = move || currency.get().order_size() prop:step = move || currency.get().tick_size()
                                        on:change = move |e| {
                                            ...
                                            if has_filled.get() == false && next_step.get() <= 6 {
                                                next_step.set(6)
                                            } else {
                                                next_step.update(|value| *value += 0)
                                            }
                                ...
                                view! {
                                    ...
                                    <select class = "select-sm text-xs text-success block w-full rounded hover:shadow-sm hover:shadow-success bg-base-100 shadow-md" name="expiry_in_min"
                                        on:change = move |event| {
                                            ...
                                            quote_expiry.set(date_time.clone());
                                            gtc_signal.set(gtc);
                ...
                //             view! {
                //                 <div class = "skeleton flex-1 sm:basis-full lg:basis-auto border rounded-md bg-base-300">
                //                 </div>
                //             }.into_view()
                //         } else {
                //             view! {
                //                 <div class = "flex-1 sm:basis-full lg:basis-auto border rounded-md bg-base-300">
                        ...
                        <ul class="steps">
                            {
                                move || {
                                    if rfq_type.get() == "Perpetual".to_string() {
                                        view! {
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 1)>Type</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 3)>Pair</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 4)>Amount</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 5)>Price</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 6)>Quote Expiry</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 7)>Counterparty</li>
                                        }.into_view()
                                    } else {
                                        view! {
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 1)>Type</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 2)>Expiry</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 3)>Pair</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 4)>Amount</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 5)>Price</li>
                                            <li class="step step-neutral" class=("step-primary", move || next_step.get() >= 6)>Quote Expiry</li>
...
//     let quote_table = RwSignal::new(Vec::<ExtractedFuturesQuote>::default());
...
//                 <div class = "flex-0">
//                     <button class = {move || if selected_page.get() == QuotesTab::Active {"btn btn-sm btn-ghost capitalize font-semibold bg-base-300 rounded border border-success text-success"} else {"btn btn-sm btn-ghost capitalize font-normal bg-base-300 rounded"}} on:click = move |_| select_page(QuotesTab::Active)>Active</button>
//                 </div>
//                 <div class = "flex-0">
//                     <button class = {move || if selected_page.get() == QuotesTab::Approved {"btn btn-sm btn-ghost capitalize font-semibold bg-base-300 rounded border border-success text-success"} else {"btn btn-sm btn-ghost capitalize font-normal bg-base-300 rounded"}} on:click = move |_| select_page(QuotesTab::Approved)>Approved</button>
//                 </div>
//                 <div class = "flex-0">
//                     <button class = {move || if selected_page.get() == QuotesTab::Rejected {"btn btn-sm btn-ghost capitalize font-semibold bg-base-300 rounded border border-success text-success"} else {"btn btn-sm btn-ghost capitalize font-normal bg-base-300 rounded"}} on:click = move |_| select_page(QuotesTab::Rejected)>Rejected</button>
//                 </div>
//             </div>
//         </div>
...
// #[component]
// pub fn ActiveQuotes(
//     quote_data: RwSignal<Vec<ExtractedFuturesQuote>>,
...
//     quotes_list: RwSignal<Vec<FuturesQuote>>,
//     quotes_resource: Resource<(), Result<FuturesQuoteHistory, ServerFnError>>,
// ) -> impl IntoView {
//     move || {
...
// #[component]
// pub fn DataTable(
//     data: RwSignal<Vec<ExtractedFuturesQuote>>,
//     show_approve_quote_alert: RwSignal<bool>,
//     approve_quote_response: RwSignal<ApproveTradeQuoteResponse>,
//     approve_trade_quotes: Action<(String, Vec<String>), ()>,
//     quotes_list: RwSignal<Vec<FuturesQuote>>,
...
//     let selected_header = RwSignal::new(QuoteSort::GroupId);
...
//                         <th class = "cursor-pointer" on:click = move |_| {selected_header.set(QuoteSort::Type); sort_asc.update(|s| *s = !*s)}>
//                             <div class = "flex justify-between">
//                                 <span class = "flex-0">Type</span>
//                                 <span  class = "flex-0">
//                                     <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
...
//                             </div>
//                         </th>
//                         <th class = "cursor-pointer" on:click = move |_| {selected_header.set(QuoteSort::Expiry); sort_asc.update(|s| *s = !*s)}>
//                             <div class = "flex justify-between">
//                                 <span class = "flex-0">Expiry</span>
//                                 <span  class = "flex-0">
//                                     <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
//                                     <path fill-rule="evenodd" d="M10 3a.75.75 0 01.55.24l3.25 3.5a.75.75 0 11-1.1 1.02L10 4.852 7.3 7.76a.75.75 0 01-1.1-1.02l3.25-3.5A.75.75 0 0110 3zm-3.76 9.2a.75.75 0 011.06.04l2.7 2.908 2.7-2.908a.75.75 0 111.1 1.02l-3.25 3.5a.75.75 0 01-1.1 0l-3.25-3.5a.75.75 0 01.04-1.06z" clip-rule="evenodd" />
//                                     </svg>
...
//                             </div>
//                         </th>
//                         <th class = "cursor-pointer" on:click = move |_| {selected_header.set(QuoteSort::Pair); sort_asc.update(|s| *s = !*s)}>
//                             <div class = "flex justify-between">
...
//                             </div>
//                         </th>
//                         <th class = "cursor-pointer" on:click = move |_| {selected_header.set(QuoteSort::Amount); sort_asc.update(|s| *s = !*s)}>
//                             <div class = "flex justify-between">
...
//                             </div>
//                         </th>
//                         <th class = "cursor-pointer" on:click = move |_| {selected_header.set(QuoteSort::Price); sort_asc.update(|s| *s = !*s)}>
//                             <div class = "flex justify-between">
...
//                             </div>
//                         </th>
//                         <th class = "cursor-pointer" on:click = move |_| {selected_header.set(QuoteSort::QuoteExpiry); sort_asc.update(|s| *s = !*s)}>
//                             <div class = "flex justify-between">
...
//                             </div>
//                         </th>
//                         <th class = "cursor-pointer" on:click = move |_| {selected_header.set(QuoteSort::Counterparty); sort_asc.update(|s| *s = !*s)}>
//                             <div class = "flex justify-between">
...
// #[server]
// pub async fn get_quotes() -> Result<FuturesQuoteHistory, ServerFnError> {
    ...
    //     }
    // }
//     Ok(FuturesQuoteHistory::default())
// }
